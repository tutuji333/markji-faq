...
- 建议统一在第二个 `#` 后保留一个普通空格，写成 `[E## \frac{a}{b}]`，便于阅读和检查。
...
处理原则：固定使用 `[Pic#ID/<原图 file.id>,MID/<遮罩 file.id>#]`；两项都从真实记录读取并逐字符核对，不能交换、留空或改写大小写。
...
本文没有收录仅在研究代码中出现、但尚未通过足够真实样本确认的 `F` 等候选标签。遇到本文未覆盖的效果时，应先从真实卡片中取得可复现样本并验证，不能让 AI 根据标签名称自行发明语法。
