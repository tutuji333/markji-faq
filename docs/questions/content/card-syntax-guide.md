...
- 段落、选择题、图片、独立公式和答案线属于块级语法，必须从新一行的第一个字符开始。
...
行内语法之间不要互相嵌套。`P`、`Choice`、`Pic`、独立 `E` 和 `---` 不要放进其他语法内部。
...
- 标签内部只写 KaTeX 支持的 LaTeX 内容；不要把说明文字、题号或整段中文强行塞进公式。
...
- 选项内只使用普通文字和 `[T#...#...]` 文字样式，不要放挖空、音频、卡片引用、图片或公式。
- 选项文字中的普通中括号使用 `\[` 和 `\]` 转义。
...
错误结果：挖空无法分组、显示异常，或者内部样式破坏标签结构。

处理原则：使用 `[F#正整数编号#内容]`，编号从 `1` 开始；相同编号表示同一组，内容中不再嵌套其他语法。
...
- [ ] 所有挖空都使用从 `1` 开始的正整数编号，分组符合预期，内容中没有嵌套其他语法。
...
- 由 KaTeX 渲染的 `[E##...]` LaTeX 公式；
- 单选与多选选择题；
- HTML 转 Markji 富文本时使用的基础样式映射。

本文没有收录官方文档未列出、且尚未通过足够真实样本确认的候选标签或参数。遇到本文未覆盖的效果时，应先查阅官方文档，或从真实卡片中取得可复现样本并验证，不能让 AI 根据标签名称自行发明语法。
